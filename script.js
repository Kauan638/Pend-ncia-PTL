...
async function carregarEnderecos(arquivoParam){
...
//
// Conecta a subpasta "Pendência PTL" (dentro da pasta mestre)
// uma única vez. A partir daí, detecta sozinho:
//   - o arquivo Velox   (extensão .xlsx / .xls / .csv)
//   - o arquivo de Posição de Endereços (extensão .txt)
// e reprocessa automaticamente sempre que qualquer um dos
// dois for salvo/atualizado no disco.
//
// Reaproveita 100% da lógica já existente acima:
// carregarEnderecos(file), lerCSV(file), lerExcel(file).
// ========================================================
// ========================================================

const SYNC_DB_NAME = "pendencia-ptl-sync-db";
const SYNC_STORE_NAME = "handles";
const SYNC_INTERVALO_MS = 5000; // checa a cada 5s

let syncDirHandle = null;
let syncArquivoVeloxHandle = null;
let syncArquivoEnderecosHandle = null;
let syncLastModifiedVelox = 0;
let syncLastModifiedEnderecos = 0;
...
async function syncSalvarHandle(handle){
    ...
    return new Promise((resolve, reject)=>{
        ...
        tx.objectStore(SYNC_STORE_NAME).put(handle, "pastaPTL");
...
async function syncCarregarHandle(){
    ...
    return new Promise((resolve, reject)=>{
        ...
        const req = tx.objectStore(SYNC_STORE_NAME).get("pastaPTL");
...
async function syncLimparHandle(){
    ...
    tx.objectStore(SYNC_STORE_NAME).delete("pastaPTL");
...
function syncSetStatus(tipo, textoExtra){
    ...
    document.getElementById("btnConectarPasta").style.display =
    tipo === "off" ? "inline-block" : "none";

    document.getElementById("btnDesconectarPasta").style.display =
    tipo === "off" ? "none" : "inline-block";
...
const SYNC_EXT_VELOX = [".xlsx",".xls",".csv"];
const SYNC_EXT_ENDERECOS = [".txt"];
...
async function syncVarrerPasta(){
    ...
    syncArquivoVeloxHandle = null;
    syncArquivoEnderecosHandle = null;
    ...
    for await (const [nome, handle] of syncDirHandle.entries()){
        ...
        if(
            !syncArquivoVeloxHandle &&
            syncTemExtensao(nome, SYNC_EXT_VELOX)
        ){

            syncArquivoVeloxHandle = handle;

        }else if(
            !syncArquivoEnderecosHandle &&
            syncTemExtensao(nome, SYNC_EXT_ENDERECOS)
        ){

            syncArquivoEnderecosHandle = handle;

        }

    }

    if(!syncArquivoVeloxHandle){

        alert(
            "Não encontrei nenhum arquivo Velox (.xlsx/.xls/.csv) " +
            "dentro dessa pasta."
        );
...
async function syncProcessarArquivos(){
    ...
    try{
        ...
        if(syncArquivoEnderecosHandle){

            const arquivoEnderecos =
            await syncArquivoEnderecosHandle.getFile();

            await carregarEnderecos(arquivoEnderecos);

        }

        const arquivoVelox =
        await syncArquivoVeloxHandle.getFile();

        const nome = arquivoVelox.name.toLowerCase();
        ...
        if(nome.endsWith(".csv")){
            ...
            lerExcel(arquivoVelox);

        }

        // reflete nos campos de nome de arquivo da UI manual também
        document.getElementById("nomeArquivo").innerText =
        "🔗 " + arquivoVelox.name + " (auto)";

        if(syncArquivoEnderecosHandle){

            document.getElementById("nomeEnderecos").innerText =
            "🔗 " + syncArquivoEnderecosHandle.name + " (auto)";
...
function syncIniciarMonitoramento(){
    ...
    const nomesDetectados = [
        ...
        syncArquivoVeloxHandle?.name,
        syncArquivoEnderecosHandle?.name
...
async function syncChecarMudancas(){
    ...
    try{
        ...
        if(fileVelox.lastModified !== syncLastModifiedVelox){

            syncLastModifiedVelox = fileVelox.lastModified;
        ...
        if(syncArquivoEnderecosHandle){
            ...
            if(
                fileEnderecos.lastModified !==
                syncLastModifiedEnderecos
            ){
...
async function conectarPastaPTL(){
    ...
    try{
        ...
        // marca os lastModified atuais como "já processado"
        // e faz a primeira carga imediatamente
        await syncProcessarArquivos();

        const fileVelox = await syncArquivoVeloxHandle.getFile();
        syncLastModifiedVelox = fileVelox.lastModified;
        ...
        if(syncArquivoEnderecosHandle){
            ...
            syncLastModifiedEnderecos = fileEnderecos.lastModified;
...
async function desconectarPastaPTL(){
    ...
    syncArquivoVeloxHandle = null;
    syncArquivoEnderecosHandle = null;
    syncLastModifiedVelox = 0;
    syncLastModifiedEnderecos = 0;

    await syncLimparHandle();

    syncSetStatus("off");

    document.getElementById("syncUltimaChecagem").style.display = "none";
